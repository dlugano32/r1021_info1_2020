...
int main(void)
{
        ...
        fp1 = fopen("fluctuaciones.txt", "w"); //Abro el archivo donde voy a guardar las anomalias en la tension
        if (fp1 == NULL)
        {
        ...
        printf("Para enviar archivos con el cliente espere a que aparezca el mensaje 'Esperando conexiones' \n\n");

        for (i = 0; i < 10; i++) //Como tengo 10 zonas hago 10 conexiones
        {
                sleep(10);      //Sin este sleep la conexion no funciona bien
        ...
        printf("Todos los datos transferidos\n");
        fclose(fp1);
        free(vec);
        shutdown_allegro();
        return 0;
